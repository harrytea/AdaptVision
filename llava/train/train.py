...
def train():
    ...
    vision_global = model.get_model().get_vision_global().to(dtype=torch.bfloat16, device=args.device)
    vision_local = model.get_model().get_vision_local().to(dtype=torch.bfloat16, device=args.device)
    ...
    args.image_processor = vision_global.image_processor
